...
def test_python_render():
    ...
    html_string = '<body><images src=pattern.png>'
    css_string = '''
        @page { margin: 2px; size: 8px; background: #fff }
        body { margin: 0; font-size: 0 }
        images { image-rendering: pixelated }

        @media screen { images { transform: rotate(-90deg) } }
    '''
...
def test_command_line_render():
    ...
    css = b'''
        ...
        body { margin: 0; font-size: 0 }
    '''
    html = b'<body><images src=pattern.png>'
...
def test_links():
    ...
    assert_links('''
        <style>
            ...
            images { width: 30px; vertical-align: top }
        </style>
        <p><a href="http://weasyprint.org"><images src=pattern.png></a></p>
        <p style="padding: 0 10px"><a
            href="#lipsum"><images style="border: solid 1px"
                                src=pattern.png></a></p>
...
def test_navigator():
    with temp_directory() as temp:
        ...
        for status, headers, body in [
            ...
            assert headers['Content-Type'].startswith('text/html;')
            assert '<title>WeasyPrint Navigator</title>' in body
            assert '<images src="data:image/png;base64,' in body
...
def test_url_fetcher():
    ...
    test('<body><images src="pattern.png">')  # Test a "normal" URL
    test('<body><images src="weasyprint-custom:foo/é_%e9_pattern">')
    ...
    with capture_logs() as logs:
        test('<body><images src="custom:foo/bar">', blank=True)
    assert len(logs) == 1
