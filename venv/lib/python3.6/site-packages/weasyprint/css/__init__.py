    ...
    for element in tree.iter():
        ...
        if element.tag == 'body':
            ...
            if element.get('color'):
                ...
                'iframe', 'applet', 'embed', 'images', 'input', 'object'):
            if (element.tag != 'input' or
                ...
                if element.get('vspace'):
                    ...
                    yield specificity, check_style_attribute(
                        element,
                        'margin-top:%s;margin-bottom:%s' % (vspace, vspace))
                # TODO: images seems to be excluded for width and height, but a
                # lot of W3C tests rely on this attribute being applied to images
                if element.get('width'):
                    style_attribute = 'width:%s' % element.get('width')
                    if element.get('width').isdigit():
                        style_attribute += 'px'
                ...
                if element.get('height'):
                    ...
                    yield specificity, check_style_attribute(
                        element, style_attribute)
                if element.tag in ('images', 'object', 'input'):
                    if element.get('border'):
                        yield specificity, check_style_attribute(
                            element,
