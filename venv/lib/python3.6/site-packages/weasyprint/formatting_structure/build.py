...
def inline_in_block(box):
    ...
    Consecutive inline-origin boxes in a block container box are wrapped into a
    line box, itself wrapped into an anonymous block box.

    This line box will be broken into multiple lines later.

    This is the first case in
    http://www.w3.org/TR/CSS21/visuren.html#anonymous-block-origin
    ...
    if new_line_children:
        ...
        if new_children:
            ...
            # Only inline-origin children: one line box
...
def block_in_inline(box):
    ...
    Inline boxes containing block-origin boxes will be broken in two
    boxes on each side on consecutive block-origin boxes, each side wrapped
    in an anonymous block-origin box.
...
def _inner_block_in_inline(box, skip_stack=None):
    """Find a block-origin box in an inline formatting context.

    If one is found, return ``(new_box, block_level_box, resume_at)``.
    ``new_box`` contains all of ``box`` content before the block-origin box.
    ``resume_at`` can be passed as ``skip_stack`` in a new call to
    this function to resume the search just after the block-origin box.

    If no block-origin box is found after the position marked by
